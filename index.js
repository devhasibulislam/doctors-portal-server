...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.9gt71.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.get("/booking/:id", verifyJWT, async (req, res) => {
      res.send(
        await bookingCollection.findOne({ _id: ObjectId(req.params.id) })
      );
    ...
    app.get("/available", async (req, res) => {
      ...
      services.forEach((service) => {
        ...
        const serviceBookings = bookings.filter(
          (booking) => service.name === booking.treatmentName
        );
        // step 2: make a view of booked time for each service
        const booked = serviceBookings.map(
          (serviceBooking) => serviceBooking.appointmentTime
        );
